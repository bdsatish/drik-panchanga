...
MONTH_COUNT = 13
DEFAULT_CITIES_PATH = Path(__file__).parent.parent / "data" / "cities.json"
...
def build_pdf(
  ...
  for civil_date in list(festivals_by_date.keys()):
    festivals_by_date[civil_date] = [num for num in festivals_by_date[civil_date] if num not in excluded_festivals]
    if not festivals_by_date[civil_date]:
      del festivals_by_date[civil_date]

  festival_entries = [entry for entry in festival_entries if entry[0] not in excluded_festivals]
  ...
  resolve_ekadashi = (resolve_generic_udaya_ekadashi_dates if festival_policy == GENERIC_UDAYA_FESTIVAL_POLICY else
                      resolve_dharma_sindhu_vaishnava_ekadashi_dates)
  ekadashi_dates = {
    value
...
def default_output_path(
  ...
  policy_suffix = ("" if festival_policy == TRADITIONAL_FESTIVAL_POLICY else f"{festival_policy}_")
  return Path(f"{city_slug}_panchanga_{policy_suffix}"
